...
RenderHTML <- function(file, outname = "", filepath = here::here("Lessons"), outputpath = here::here("HTML"), replace = FALSE){
  ...
  command <- paste0("pandoc ", filepath, "/", file, " -o ", outputpath, "/", outname, " -t html5")
  shell(command)
}
